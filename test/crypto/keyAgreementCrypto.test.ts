...
describe('Key Agreement Crypto API wrapper test\n', () => {
  ...
  beforeAll(() => {
    sharedSecret = helper.randomSecret(10)
    r1 = new BN(
  ...
  it('ComputeXi with 2 users should equal 1', () => {
    ...
    const x1 = keyAgreementCrypto.computeXi(r1, z2, z2)
    const x2 = keyAgreementCrypto.computeXi(r2, z1, z1)
  ...
  it('keyAgreement protocol with 2 users should succeed', async () => {
    ...
    const sk1 = keyAgreementCrypto.computeSKi(r1, x1, z2, xiList)
    const sk2 = keyAgreementCrypto.computeSKi(r2, x2, z1, xiList)
  ...
  it('keyAgreement protocol with 3 users should succeed', async () => {
    ...
    const sk1 = keyAgreementCrypto.computeSKi(r1, x1, z3, xiList)
    const sk2 = keyAgreementCrypto.computeSKi(r2, x2, z1, xiList)
    const sk3 = keyAgreementCrypto.computeSKi(r3, x3, z2, xiList)
  ...
  it('keyAgreement protocol with 4 users should succeed', async () => {
    ...
    const z2 = keyAgreementCrypto.computeZi(r2)
    const z3 = keyAgreementCrypto.computeZi(r3)
    const z4 = keyAgreementCrypto.computeZi(r4)

    const x1 = keyAgreementCrypto.computeXi(r1, z2, z4)
    const x2 = keyAgreementCrypto.computeXi(r2, z3, z1)
    const x3 = keyAgreementCrypto.computeXi(r3, z4, z2)
    const x4 = keyAgreementCrypto.computeXi(r4, z1, z3)

    const xiList = [x1.clone(), x2.clone(), x3.clone(), x4.clone()]

    const sk1 = keyAgreementCrypto.computeSKi(r1, x1, z4, xiList)
    const sk2 = keyAgreementCrypto.computeSKi(r2, x2, z1, xiList)
    const sk3 = keyAgreementCrypto.computeSKi(r3, x3, z2, xiList)
    const sk4 = keyAgreementCrypto.computeSKi(r4, x4, z3, xiList)
