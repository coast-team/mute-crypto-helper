...
import { g, KDFDeriveKeyParams, KDFKeyParams, p, riSize } from './helper/keyAgreementCryptoHelper'
...
export function generateRi() {
  return new BN(global.crypto.getRandomValues(new Uint8Array(riSize)) as Buffer)
}
 ...
 */
export function computeXi(ri: BN, ziRight: BN, ziLeft: BN) {
 ...
 */
export function computeSharedSecret(ri: BN, xi: BN, ziLeft: BN, xiList: BN[]) {
  const res = ziLeft.toRed(red).redPow(ri.mul(new BN(xiList.length)) as any)
