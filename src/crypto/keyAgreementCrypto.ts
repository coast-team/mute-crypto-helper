...
import { g, KDFDeriveKeyParams, KDFKeyParams, red } from './helper/keyAgreementCryptoHelper'
...
export function computeZi(ri: Uint8Array) {
  return g.redPow(new BN(ri as Buffer)).fromRed()
}
 ...
 */
export function computeXi(ri: Uint8Array, ziRight: any, ziLeft: any) {
  const modInvM = ziLeft.toRed(red).redInvm()
  return modInvM
    .redMul(ziRight.toRed(red))
    .redPow(new BN(ri as Buffer))
    .fromRed()
}
 ...
 */
export function computeSKi(ri: any, xi: any, ziLeft: any, xiList: any[]) {
  const res: any = ziLeft.toRed(red).redPow(new BN(ri as Buffer).mul(new BN(xiList.length)))
  const iInit = indexOf(xiList, xi)
  const exponent: any = new BN(xiList.length - 1)
  let xin: any
  let ind: number
  for (let i = 0; exponent > 0; i++) {
    ind = (iInit + i) % xiList.length
    xin = xiList[ind].toRed(red).redPow(exponent)
    res.redIMul(xin)
    exponent.isubn(1)
  }
  return res.fromRed()
...
function indexOf(xiList: any[], xi: any) {
  let indexOfXi = -1
